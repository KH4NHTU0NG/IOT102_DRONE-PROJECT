...
3. **Demo & Phát triển (Không cần mạch thật):**
   Nếu bạn chưa mua linh kiện hoặc không mang mạch theo, bạn hoàn toàn có thể chạy file giả lập. Kịch bản này sẽ sinh ra dữ liệu nhiệt độ, khí gas giả lập và gửi lên MQTT giống hệt như một board BW16 thật. Dùng để test Web Dashboard và Grafana ngay trên máy tính cục bộ. Mở Terminal mới và gõ: `bash Phase5_Operations/run_mock.sh`.
