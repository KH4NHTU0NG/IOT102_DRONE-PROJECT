...
def check_continuity():
    ...
    # Query co2 timestamp to check continuity
    query = f'''
    ...
    try:
        ...
        if len(times) < 2:
            print(f"[TEST] ⚠️ Không đủ dữ liệu để tính toán khoảng cách (số điểm = {len(times)})")
            return False, 1.0
        ...
        print(f"[TEST] Kiểm tra: {len(times)} điểm, phát hiện {gaps} điểm bị trống (gap > 3s). Tỷ lệ lỗi: {gap_ratio*100:.2f}%")
        
        # Đạt yêu cầu nếu tỷ lệ khoảng cách trống < 5%
        is_pass = gap_ratio < 0.05
        return is_pass, gap_ratio

    except Exception as e:
        print(f"[TEST] ❌ Lỗi khi truy vấn InfluxDB: {e}")
        return False, 1.0
    finally:
        client.close()
...
def main():
    print("=" * 60)
    print("  test_continuity.py — Khởi động kiểm thử tính liên tục")
    print("  Chạy trong 2 phút, lấy mẫu mỗi 30 giây...")
    print("=" * 60)
    ...
    for round_num in range(1, total_rounds + 1):
        print(f"\n▶ Round {round_num}/{total_rounds} (Chờ 30 giây để lấy mẫu...)")
        time.sleep(30)
    ...
    print(f"KẾT QUẢ CUỐI CÙNG: {passed_rounds}/{total_rounds} rounds thành công ({success_rate:.1f}%)")
